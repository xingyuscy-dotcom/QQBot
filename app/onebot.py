...
from .llm_client import GENERAL_FALLBACK_REPLY, LLMError, generate_reply_result
...
async def handle_group_message(websocket: WebSocket, event: dict, bot_qq: str, text: str) -> None:
    ...
    try:
        ...
        await send_group_message(websocket, group_id, GENERAL_FALLBACK_REPLY)
        save_conversation_message(bot_qq, "group", group_id, bot_qq, None, GENERAL_FALLBACK_REPLY, is_bot=True)
        return
    except Exception as exc:
    ...
    reply = result.reply
    if not reply:
        log_event("info", f"group:{bot_qq}:{group_id}", "ai reply skipped because model returned empty content")
        return
...
async def handle_private_message(websocket: WebSocket, event: dict, bot_qq: str, text: str) -> None:
    ...
    try:
        ...
        await send_private_message(websocket, user_id, GENERAL_FALLBACK_REPLY)
        save_conversation_message(bot_qq, "private", user_id, bot_qq, None, GENERAL_FALLBACK_REPLY, is_bot=True)
        return
    except Exception as exc:
    ...
    reply = result.reply
    if not reply:
        log_event("info", f"private:{bot_qq}:{user_id}", "ai reply skipped because model returned empty content")
        return
